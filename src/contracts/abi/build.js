...
const contractsPath = path.join(rootPath, 'src', 'contracts', 'abi');
...
async function clean() {
	...
	await Promise.all(
		filenames
			.filter((filename) => {
				return /ABI.js(on)?$/.test(filename);
			})
			.map((filename) => {
...
async function build() {
	...
	for (const [name, address] of Object.entries(contractAddresses)) {
		...
		const fileContents = `export default ${dataFormatted};`;
		const filename = `${name}ABI.js`;
		await fs.writeFile(path.join(contractsPath, filename), fileContents, 'utf8');
